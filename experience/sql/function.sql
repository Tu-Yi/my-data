...
-- 练习
select LAST_NAME,EMPLOYEE_ID,SALARY,ROUND(SALARY*1.15) AS "New SALARY" from EMPLOYEES
select INITCAP(LAST_NAME),LENGTH(LAST_NAME) from EMPLOYEES WHERE LAST_NAME like 'J%' OR LAST_NAME LIKE 'M%' OR LAST_NAME LIKE 'A%' ORDER BY LAST_NAME
SELECT LAST_NAME, ROUND(MONTHS_BETWEEN(SYSDATE,HIRE_DATE))  FROM EMPLOYEES ORDER BY MONTHS_BETWEEN(SYSDATE,HIRE_DATE)
SELECT LAST_NAME || 'EARNS' || TO_CHAR(SALARY,'fm$999,999.00') || 'month but wants' || TO_CHAR(3*SALARY,'fm$999,999.00') as "Dream sa" FROM EMPLOYEES
SELECT LAST_NAME, LPAD(SALARY, 15, '$') as  SALARY from EMPLOYEES
select LAST_NAME, TO_CHAR(NEXT_DAY(ADD_MONTHS(HIRE_DATE, 6),2),'yyyy-mm-dd day') REVIEW, SALARY from EMPLOYEES
SELECT LAST_NAME,HIRE_DATE,TO_CHAR(HIRE_DATE,'DAY') DAY FROM EMPLOYEES ORDER BY TO_CHAR(HIRE_DATE-1,'d');
SELECT LAST_NAME,NVL(TO_CHAR(COMMISSION_PCT), 'No Commmission') from EMPLOYEES
select JOB_ID,
DECODE(JOB_ID, 
'AD_PRES','A', 
'ST_MAN','B', 
