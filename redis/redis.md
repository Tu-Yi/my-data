...
http://download.redis.io/releases/下载对应版本的conf
...
![](https://niliv-technology-1252830662.cos.ap-chengdu.myqcloud.com/sql/redis%E5%A4%A7%E7%BA%B2%E6%88%AA%E5%9B%BE.jpg)

![](https://niliv-technology-1252830662.cos.ap-chengdu.myqcloud.com/sql/redis%E5%A4%A7%E7%BA%B2%E6%88%AA%E5%9B%BE2.jpg)
...
- 从大家熟知的mysql出发来认识redis
  ...
  - 图解剖析mysql的执行过程
    ...
    ![](https://niliv-technology-1252830662.cos.ap-chengdu.myqcloud.com/sql/mysql%E6%89%A7%E8%A1%8C%E6%AD%A5%E9%AA%A4%E5%9B%BE.png)

    ![](https://niliv-technology-1252830662.cos.ap-chengdu.myqcloud.com/sql/mysql%E6%89%A7%E8%A1%8C%E7%BB%86%E8%8A%82.png)

### Redis和memcached和mysql之间的区别

![](https://niliv-technology-1252830662.cos.ap-chengdu.myqcloud.com/sql/redis%E4%B8%8E%E5%85%B6%E4%BB%96%E6%95%B0%E6%8D%AE%E5%BA%93%E5%8C%BA%E5%88%AB.jpg)
...
- 图解分析加redis前后的架构区别
 ...
 ![](https://niliv-technology-1252830662.cos.ap-chengdu.myqcloud.com/sql/NOSQL%E5%B8%A6%E6%9D%A5%E7%9A%84%E6%94%B9%E5%8F%98.jpg)
...
- 首先，检查CentOS7是否安装了openssh-server，在终端中输入 yum list installed | grep openssh-server 此处显示已经安装了 openssh-server，如果又没任何输出显示表示没有安装，通过输入yum install openssh-server安装
...
- 其他命令
  ...
  - append：给指定key的value追加字符串，并返回新字符串的长度
     ...
     ```bash
    [root@test yuankun]# docker exec -it myredis /bin/bash
    root@ec94969fb402:/data# redis-cli
    ...
    (error) ERR invalid password
    127.0.0.1:6379> auth HM@dmmmbbmm!1314
    (error) ERR invalid password
    127.0.0.1:6379> auth 'HM@dmmmbbmm!1314'
    (error) ERR invalid password
    127.0.0.1:6379> auth yuan@Kun52011
    OK
    127.0.0.1:6379> get name
    "yuankun"
    ...
    (error) ERR unknown command `mget[name,age]`, with args beginning with: 
    127.0.0.1:6379> mget[name age]
    (error) ERR unknown command `mget[name`, with args beginning with: `age]`, 
    127.0.0.1:6379> mgetname age
    (error) ERR unknown command `mgetname`, with args beginning with: `age`, 
    127.0.0.1:6379> mget name age
    1) "yuankun"
    2) "35"
    ...
    127.0.0.1:6379> incby age 40
    (error) ERR unknown command `incby`, with args beginning with: `age`, `40`, 
    127.0.0.1:6379> incrby age 40
    (integer) 78
    127.0.0.1:6379> incrby age 40
    (integer) 118
    127.0.0.1:6379> setex age1 30 10
    OK
...
类似于HashMap，Map<String,Map<String,String>>
...
- Hash命令讲解
  ...
  - hlen--返回key对应的HashMap中的field的数量
    ...
    (integer) 1
    127.0.0.1:6379> hset myinfo name niliv
    (integer) 1
    127.0.0.1:6379> hset myinfo salary 20000
    (integer) 1
    127.0.0.1:6379> get myinfo
    (error) WRONGTYPE Operation against a key holding the wrong kind of value
    127.0.0.1:6379> hget  myinfo
    (error) ERR wrong number of arguments for 'hget' command
    127.0.0.1:6379> hget  myinfo name
    "niliv"
    127.0.0.1:6379> hget  myinfo age name
    (error) ERR wrong number of arguments for 'hget' command
    ...
    5) "salary"
    6) "20000"
    127.0.0.1:6379> hmset myinfo1 age1 40 name1 niliv1 salary 10000
    OK
    127.0.0.1:6379> hget myinfo1
    (error) ERR wrong number of arguments for 'hget' command
    127.0.0.1:6379> hmget myinfo1
    (error) ERR wrong number of arguments for 'hmget' command
    127.0.0.1:6379> hmget myinfo1 name
    1) (nil)
    127.0.0.1:6379> hmget myinfo1 name1
    1) "niliv1"
...
- rpop——从key对应的list的尾部删除一个元素，并返回该元素
 ...
 ```bash
127.0.0.1:6379> lpush newlist value1 value2 value3
(integer) 3
127.0.0.1:6379> lrange newlist 0
...
1) "value2"
2) "value1"
127.0.0.1:6379> rpush newlist value3
(integer) 3
127.0.0.1:6379> lrange newlist 0 2
1) "value2"
2) "value1"
...
2) "value1"
3) "value3"
127.0.0.1:6379> lpush newlist haha
(integer) 4
127.0.0.1:6379> lrange newlist 0 3
1) "haha"
2) "value2"
...
![](https://niliv-technology-1252830662.cos.ap-chengdu.myqcloud.com/sql/%E8%B7%B3%E8%B7%83%E8%A1%A81.png)
...
- - Redis 是一个开源（BSD许可）的，内存中的数据结构存储系统，它可以用作数据库、缓存和消息中间件。 它支持多 种类型的数据结构，如 字符串（strings）、散列（hashes）、 列表（lists）、 集合（sets）、 有序集合（sorted sets）等。
    ...
    ```bash
    127.0.0.1:6379> zadd myzset 1 "one" 2 "two" 3 "threee"
    (integer) 3
    ...
    1) "two"
    2) "threee"
    127.0.0.1:6379> zrangebyscore myzset2 3
    (error) ERR wrong number of arguments for 'zrangebyscore' command
    127.0.0.1:6379> zrangebyscore myzset 2 3
    1) "two"
    2) "threee"
    127.0.0.1:6379> zadd myzset 5 "five"  6 "six"
    (integer) 2
    127.0.0.1:6379> zrangebyscore myzset 2 5
    1) "two"
    2) "threee"
    3) "five"
    ...
    127.0.0.1:6379> zrank myzset six
    (integer) 3
    127.0.0.1:6379> zrank myzset threee
    (integer) 1
    127.0.0.1:6379> zrank myzset two
    (integer) 0
